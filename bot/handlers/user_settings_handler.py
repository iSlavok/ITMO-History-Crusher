...
async def open_settings(callback: CallbackQuery, user: User):
    await callback.message.answer(
        ...
        Кол-во вариантов ответа: {user.suggested_answers_count}
...
async def open_answer_count(callback: CallbackQuery):
    await callback.message.answer("Выберите кол-во вариантов ответа", reply_markup=get_settings_answer_count_kb())
    await callback.answer()
...
async def set_answer_count(callback: CallbackQuery, callback_data: SettingAnswerCountCD, user_service: UserService):
    ...
    await callback.message.answer(f"Установлено кол-во вариантов ответа: {count}",
